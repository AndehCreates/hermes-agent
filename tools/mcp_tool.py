    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        try:
            ...
            try:
                parsed = json.loads(result)
                if "error" in parsed:
                    # Check for session-invalidated errors and reconnect if found
                    err_msg = str(parsed.get("error", ""))
                    if "session" in err_msg.lower():
                        logger.info(
                            "MCP server '%s': session invalidated — triggering reconnect",
                            server_name,
                        )
                        with _lock:
                            srv = _servers.get(server_name)
...
def _make_list_resources_handler(server_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        try:
            result = _call_once()
            # Check if the MCP server returned a session error
            try:
                parsed = json.loads(result)
                if "error" in parsed:
                    err_msg = str(parsed.get("error", ""))
